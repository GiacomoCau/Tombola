...
I numeri a disposizione per formare le schede o cartelle sono 90 (da 1 a 90)

<ul style="list-style-type:none;padding-left:0px">
<li>Con tutti e 90 i numeri va costruito un foglio composto di 6 schede
	<ul style="list-style-type:none">
		<li>ogni scheda con 15 numeri disposti in 3 righe e 9 colonne.
		<ul style="list-style-type:none">
...
<ul style="list-style-type:none;padding-left:0px">
<li>Per il foglio, ovvero complessivamante per le 6 schede,
	<ul style="list-style-type:none">
...
Di conseguenza, per quantita di numeri nelle colonne, le schede sono raggruppabili in 4 tipologie (a, b, c, d)
...
Mentre, per quantità di numeri nelle colonne del foglio, ovvero di tutte e 6 le schede, abbiamo invece
<ul style="list-style-type:none">
	<li>tipologie per la 1ª colonna (a1, a2)
	<li>tipologie per le colonne dalla 2ª all'8ª (b1, b2, b3)
	<li>tipologie per la 9ª colonna (c1, c2, c3)
</ul>
...
Dato che i numeri in ogni colonna sono determinati, 
le combinazioni delle tipologie per le 6 schede 
...
Esiste un modo per generare tutte le tipologie delle 6 schede?<br>
E da queste tutte le possibili schede?

La risposta è si (vedi Schede.{js|html} <code>F(6,3,5,[9,10,10,10,10,10,10,10,11])</code>)
ma la generazione seriale dei fogli fa in modo che un foglio sia diverso dal precedente
per la posizione di un solo numero su di una sola riga di una sola scheda,
così che tra un foglio ed il successivo vi siano ben cinque schede identiche,
rendendo i fogli generati inutilizzabili per il gioco.

Una prima alternativa, altrettanto esaustiva, potrebbe essere la generazione casuale di un foglio fra tutti i possibili
ma con la difficoltà di dover gestire una dimensione del problema veramente enorme.
Tenedo conto che esistono 735.210 strutture per una singola scheda e dovendo generare un foglio di 6 schede
ci si troverebbe a dover combattere con qualcosa, tenendo anche conto dei vincoli di numericià,
di non molto inferiore a 735.210<sup>6</sup> strutture di fogli diverse.

Una seconda alternativa, non esaustiva però, è quella di generare un foglio scegliendo a caso la prima scheda fra tutte le 735.210 strutture
(vedi Schede.{js|html}|Core.java <code>S(3,5,[3,3,3,3,3,3,3,3,3])</code>) e le successive cinque fra le strutture di schede
che siano compatibli con i vincoli verticali di numericita minima e massima di ogni colonna dettati,
di volta in volta, dalle precedenti strutture di schede scelte. 
Avendo raggruppato preventivamente le schede per numericità verticale si riesce a generare un foglio in circa 2ms,
effettuando delle semplici selezioni casuali dall'insieme delle strutture compatibili
ottenute filtrando accortamente i 1.554 raggruppamenti delle 735.210 strutture.

Una volta definite le strutture delle schede per il singolo foglio
è possibile disporre i numeri sulle schede prendendoli da una disposizione casuale per ogni decina.
Questa disposizione casuale può essere mantenuta per tutti i fogli o cambiata di volta in volta per ogni foglio generato.
...
con
	...
	pb: fine pagina, numero di separatori verticali
